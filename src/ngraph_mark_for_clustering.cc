...
Status MarkForClustering(Graph* graph) {
...

  {
    ...
    if (!initialized) {
      ...
      type_constraint_map["QuantizedConv2DWithBiasAndReluAndRequantize"]["Tinput"] = NGraphSupportedQuantizedDTypes();
      type_constraint_map["QuantizedConv2DWithBiasAndReluAndRequantize"]["Tfilter"] = NGraphSupportedQuantizedDTypes();
      ...
      set_attributes_map["QuantizedConv2DWithBiasAndReluAndRequantize"] = SetStaticInputs({3,4,5,6,7,8});
